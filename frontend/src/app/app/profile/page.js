...
export default function ProfilePage() {
  ...
  const refreshAgent = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/agents/hl?owner=${owner}`, {
        cache: 'no-store',
      });
      if (res.ok) {
        const data = await res.json();
        if (
          data &&
          (data.agent_address || data.agent_name || data.expiry_unix)
        ) {
          setAgent(data);
        } else {
          setAgent(null);
        ...
        const resList = await fetch(`/api/agents?owner=${owner}`, {
          cache: 'no-store',
        });
  ...
  const handleSignIn = async () => {
    try {
      setAuthBusy(true);
      setError(null);
      const n = await fetch('/api/auth/nonce', { cache: 'no-store' }).then(
        (r) => r.json()
      );
  ...
  const approveAgent = async () => {
    try {
      ...
      const { domain, types, primaryType, message } = buildEip712ForApprove(
        action,
        chainId
      );
      ...
      if (recovered.toLowerCase() !== owner.toLowerCase()) {
        throw new Error(
          `Signature recovers to ${recovered}, not your wallet ${owner}`
        );
      }
  ...
  const revokeAgent = async () => {
    try {
      ...
      const j = await resp.json().catch(() => ({}));
      if (!resp.ok)
        throw new Error(j?.detail || j?.response || 'Revoke failed');
      ...
      {!mounted ? (
        ...
        <Card>
          <CardContent className="p-6 space-y-4">
            <p className="text-muted-foreground">
              You’re connected as <span className="font-mono">{owner}</span>.
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      ...
                      <div className="text-sm">
                        {agent.expiry_unix
                          ? stableUTCFromSeconds(agent.expiry_unix)
                          : '—'}
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-2 space-y-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <TextInput
                        value={agentPriv}
                        onChange={(e) => setAgentPriv(e.target.value)}
                      />
                      <div className="text-xs text-muted-foreground">
                        Generated locally. Stored encrypted by backend after
                        approval. Keep it private.
                      </div>
                    </div>
                    <div className="space-y-2">
