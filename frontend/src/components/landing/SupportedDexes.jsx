    ...
    <section className="container space-y-6 py-8 md:py-12 lg:py-24">
      <div className="mx-auto flex max-w-[58rem] flex-col items-center space-y-4 text-center">
        ...
        <p className="max-w-[85%] leading-normal text-muted-foreground sm:text-lg sm:leading-7">
          Access funding data from the top perpetual DEXes across multiple chains
        </p>
      </div>
      
      <div className="mx-auto max-w-[64rem] space-y-8">
        <div>
          <h3 className="text-xl font-semibold mb-4 text-center">Supported Chains</h3>
        ...
        <div>
          <h3 className="text-xl font-semibold mb-4 text-center">Integrated DEXes</h3>
          <div className="flex flex-wrap justify-center gap-2">
            {dexes.map((dex, index) => (
              <Badge key={index} variant="secondary" className="text-sm py-1">
                {dex.name} <span className="text-muted-foreground ml-1">• {dex.chain}</span>
              </Badge>
            ))}
          </div>
        </div>
