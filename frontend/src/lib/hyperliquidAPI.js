...
export const useHyperliquidMarkets = () => {
  ...
  const hasMarketChanged = useCallback((newMarket, oldMarket) => {
    ...
    const criticalFields = ['markPrice', 'change24h', 'volume24h', 'fundingRate', 'midPrice'];
    ...
    try {
      ...
      setData(prevData => {
        ...
        formattedData.forEach(newMarket => {
          ...
          const existingIndex = prevData.findIndex(p => p.symbol === uniqueKey);
...
export const useHyperliquidFunding = () => {
  ...
  const hasFundingChanged = useCallback((newFunding, oldFunding) => {
    ...
    const criticalFields = ['fundingRate', 'annualizedRate', 'markPrice', 'openInterest'];
    ...
    try {
      ...
      setData(prevData => {
        ...
        formattedFunding.forEach(newFunding => {
          ...
          const existingIndex = prevData.findIndex(f => f.coin === uniqueKey);
